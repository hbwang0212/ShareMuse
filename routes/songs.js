var express = require("express");
var router  = express.Router();
var Song = require("../models/song");
var Comment = require("../models/comment");
var middleware = require("../middleware");
var geocoder = require('geocoder');
var { isLoggedIn, checkUserSong, checkUserComment, isAdmin, isSafe } = middleware; // destructuring assignment

// Define escapeRegex function for search feature
function escapeRegex(text) {
    return text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
};

//INDEX - show all songs
router.get("/", function(req, res){
  if(req.query.search && req.xhr) {
      const regex = new RegExp(escapeRegex(req.query.search), 'gi');
      // Get all songs from DB
      Song.find({name: regex}, function(err, allSongs){
         if(err){
            console.log(err);
         } else {
            res.status(200).json(allSongs);
         }
      });
  } else {
      // Get all songs from DB
      Song.find({}, function(err, allSongs){
         if(err){
             console.log(err);
         } else {
            if(req.xhr) {
              res.json(allSongs);
            } else {
              res.render("songs/index",{songs: allSongs, page: 'songs'});
            }
         }
      });
  }
});

//CREATE - add new song to DB
router.post("/", isLoggedIn, isSafe, function(req, res){
  // get data from form and add to songs array
  var name = req.body.name;
  var image = req.body.image;
  var desc = req.body.description;
  var author = {
      id: req.user._id,
      username: req.user.username
  }
  var cost = req.body.cost;
  geocoder.geocode(req.body.location, function (err, data) {
    if (err || data.status === 'ZERO_RESULTS') {
      req.flash('error', 'Invalid address');
      return res.redirect('back');
    }
    var lat = data.results[0].geometry.location.lat;
    var lng = data.results[0].geometry.location.lng;
    var location = data.results[0].formatted_address;
    var newSong = {name: name, image: image, description: desc, cost: cost, author:author, location: location, lat: lat, lng: lng};
    // Create a new song and save to DB
    Song.create(newSong, function(err, newlyCreated){
        if(err){
            console.log(err);
        } else {
            //redirect back to song page
            console.log(newlyCreated);
            res.redirect("/songs");
        }
    });
  });
});

//NEW - show form to create new song
router.get("/new", isLoggedIn, function(req, res){
   res.render("songs/new"); 
});

// SHOW - shows more info about one song
router.get("/:id", function(req, res){
    //find the song with provided ID
    Song.findById(req.params.id).populate("comments").exec(function(err, foundSong){
        if(err || !foundSong){
            console.log(err);
            req.flash('error', 'Sorry, that song does not exist!');
            return res.redirect('/songs');
        }
        console.log(foundSong)
        //render show template with that song
        res.render("songs/show", {song: foundSong});
    });
});

// EDIT - shows edit form for a song
router.get("/:id/edit", isLoggedIn, checkUserSong, function(req, res){
  //render edit template with that song
  res.render("songs/edit", {song: req.song});
});

// PUT - updates song in the database
router.put("/:id", isSafe, function(req, res){
  geocoder.geocode(req.body.location, function (err, data) {
    var lat = data.results[0].geometry.location.lat;
    var lng = data.results[0].geometry.location.lng;
    var location = data.results[0].formatted_address;
    var newData = {name: req.body.name, image: req.body.image, description: req.body.description, cost: req.body.cost, location: location, lat: lat, lng: lng};
    Song.findByIdAndUpdate(req.params.id, {$set: newData}, function(err, song){
        if(err){
            req.flash("error", err.message);
            res.redirect("back");
        } else {
            req.flash("success","Successfully Updated!");
            res.redirect("/songs/" + song._id);
        }
    });
  });
});

// DELETE - removes song and its comments from the database
router.delete("/:id", isLoggedIn, checkUserSong, function(req, res) {
    Comment.remove({
      _id: {
        $in: req.song.comments
      }
    }, function(err) {
      if(err) {
          req.flash('error', err.message);
          res.redirect('/');
      } else {
          req.song.remove(function(err) {
            if(err) {
                req.flash('error', err.message);
                return res.redirect('/');
            }
            req.flash('error', 'Song deleted!');
            res.redirect('/song');
          });
      }
    })
});

module.exports = router;

